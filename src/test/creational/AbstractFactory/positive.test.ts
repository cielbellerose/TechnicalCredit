...
describe('Abstract Factory: positive', () => {
  test('GUIFactory → TC (top-level factory interface implemented by WinFactory/MacFactory, each producing a matched product family)', async () => {
    const result = await analyseConstruct('GUIFactory', 'abstract', MOCK_FILE);

    expect(result.is_tc_candidate).toBe(true);
    expect(result.category).toBe('abstract');
  });

  test('Button → TC (abstract product interface implemented by WinButton/MacButton, one per concrete factory)', async () => {
    const result = await analyseConstruct('Button', 'abstract', MOCK_FILE);
  ...
  test('Checkbox → TC (second abstract product interface completing the family GUIFactory produces)', async () => {
    const result = await analyseConstruct('Checkbox', 'abstract', MOCK_FILE);
